...
<cfoutput>
	...
	<h1>PayPal In-Context Checkout Test</h1>

	<cfif application.fc.lib.paypal.hasConfiguration(endpointType="sandbox")>

		<p>This is a basic Paypal In-Context Checkout test form.</p>
		...
		<cfif url.paypalReturn eq 0 AND url.paypalCancel eq 0>
			...
			<p>
				...
				The <strong>buyer note</strong> and <strong>shipping</strong> options will be turned off.
			</p>
			<br>
			...
			<p>
				<a id="checkoutbutton" class="paypal-button-hidden" href="https://www.sandbox.paypal.com/checkoutnow?useraction=commit&token=#stResponse.TOKEN#" style="border:none;">
			...
			<script>
				window.paypalCheckoutReady = function () {
					paypal.checkout.setup('#application.fc.lib.paypal.getEndpointMerchantID(endpointType="sandbox")#', {
						environment: 'sandbox',
		...
		<cfelseif url.paypalReturn eq 1 AND url.paypalCancel eq 0>
			...
			<!--- log api request --->
			<cfset logUUID = application.fc.lib.paypal.logAPIRequest(endpointType="sandbox", description="PayPal In-Context Checkout Test", requestData=stRequestData)>
